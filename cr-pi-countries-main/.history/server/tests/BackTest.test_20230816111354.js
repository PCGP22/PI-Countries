...
describe('Prueba de rutas', () => {
    ...
    test('Ruta "Crear Actividad"', async() => {
        const placeHolderActivity = {name: "Actividad 1", difficulty: 3, duration: 2, season: "Otoño", countryId: ["KEN"] }
        const response = await request(server).post("/countries/activities/", placeHolderActivity);
        // expect(response.body).toHaveLength(1);
        expect(response.statusCode).toBe(200);
        expect(response.body).toEqual(expect.arrayContaining([ placeHolderActivity,]));
    })
