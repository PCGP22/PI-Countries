...
describe('Prueba de base de Datos',() => {
    ...
    test("Debe retornar error si se sube una actividad sin información completa",()=>{
        expect(
            async()=>{
                ...
                try{
                    await client.query('BEGIN');
                    await client.query(`INSERT INTO "Activities" (name, difficulty) VALUES ('Actividad 1', 5) RETURNING *`);
                    await client.query('ROLLBACK');
                }
                catch(error){
                    throw(error)
        ...
        ).rejects.toThrow("null value in column \"duration\" of relation \"Activities\" violates not-null constraint")
    })
});