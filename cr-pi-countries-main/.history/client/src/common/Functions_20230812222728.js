export const validar = (inputs) =>{
    ...
    console.log(inputs.name.toString().join(""))

    if(inputs.name.length < 5 || !regexName.test(inputs.name.join("")) || inputs.name.length>30){
        errores.name = "Name must be 5 to 30 characters long"
    } else if(inputs.name.length >= 5 && regexName.test(inputs.name.join("")) && inputs.name.length<30){delete errores.name}
