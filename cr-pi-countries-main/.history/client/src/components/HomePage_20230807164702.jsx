  ...
  return (
    <>
        ...
        <main className='homePage__main'>
            ...
            {currentPage && <button onClick={()=>setCurrentPage(1)} className='button'>1</button>}
            {(currentPage >4 && pages.length>13) && <span>...</span>}
            {currentPages.filter(p=>p>0 && p<=max).map(p=>{
              return(
                <button onClick={handlePageChange} className={currentPage === p ? "pagination__current--button button": "button"} key={p}>{p}</button>
              )
            })}
            {(currentPage + 8) < max && <>
            <span>...</span>
