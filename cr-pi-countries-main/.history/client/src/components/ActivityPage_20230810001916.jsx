                ...
                {activities.length > 0 && activities.map(a=>{
                    ...
                    const parse = async(url) => {
                        let file = await fetch(url)
                            .then(r => r.blob())
                            .then(blobFile => new File([blobFile], "Img", { type: "image/png" }))
                        return file
                    }
                    if(a.imageFile){imageFileSrc = parse(a.imageFile); console.log(imageFileSrc.state)}
                    return(
                        <div className='activityBox' key={a.id}>
                            <h3>Name: {a.name? a.name: "N/A"}</h3>
                            {a.imageURL && <img src={a.imageURL} width={150}/>}    
                            {a.imageFile && <img src={imageFileSrc} width={150}/>} 
                            <p> Difficulty: 
