...
const validar = (inputs) =>{
    ...
    if(inputs.imageFile && (inputs.imageFile.size > 15360 || !imageTypes.includes(inputs.imageFile.slice(-4)))){
        console.log(inputs.imageFile.slice(-4))
        errores.imageFile = "Must be a valid image file (.png, .jpg .gif, .svg)"
    } else if(!inputs.imageFile || (inputs.imageFile.size < 15360 &&  imageTypes.includes(inputs.imageFile.type))){
        delete errores.imageFile
    }
...
function ActivityPage(props) {
    ...
    const handleImgClose = () => {
        ...
        setErrores(validar({...form, imageFile: null}))
    }
        ...
        <aside className='activityPage__aside'>
            ...
            <form onSubmit={handleSubmit} className='activityPage__form'>
                ...
                <fieldset className='activityPage__formSection'>
                    ...
                    <br/>
                    {(form.imageURL === "" && form.imageFile === null)?
                        <>
                        ...
                        <>
                            <input type="text" name="imageURL" onChange={handleChange} placeholder='Image URL'/>
                            <br/>
                            <img src={`${form.imageURL}`} width={200}/>
                            <br/>
                        </>: (form.imageURL === "" && form.imageFile !== null) && 
                        <>
                            <input type="file" name='imageFile' onChange={handleChange}/>
                            <br/>
                            <div>
                                <img src={`${URL.createObjectURL(form.imageFile)}`}  width={200}/>
                                <button className='activityPage__imgCloseButton' onClick={handleImgClose}>X</button>
                            </div>
                            <br/>
                        </>   
                    }
                    {errores.imageURL?<p className="form--error no-space">{errores.imageURL}</p>:""}
                    {errores.imageFile?<p className="form--error no-space">{errores.imageFile}</p>:""}
                </fieldset>
                <fieldset className='activityPage__formSection'>
            ...
            <section className='activityContainer'>
                {activities.length > 0 && activities.map(a=>{
                    ...
                    // }
                        ...
                        // };
                    // if(a.imageFile !== null){
                    //     console.log(a.imageFile)
                    //     // let base64String = btoa(String.fromCharCode(...new Uint8Array(a.imageFile.data.toString())));
                    //     // src = `data:image/png;base64,${base64String}`
                    //     src =  'data:image/png;base64,' + arrayBufferToBase64(a.imageFile.data); //data.data in your case
                    ...
                    return(
                        <div className='activityBox' key={a.id}>
                            ...
                            {/* {a.imageFile && <img src={URL.createObjectURL(a.imageFile)} width={150}/>}  */}
                            <p> Difficulty: 
