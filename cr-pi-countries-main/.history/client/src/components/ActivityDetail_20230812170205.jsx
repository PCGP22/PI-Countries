...
import { activityCreate, addActivities, removeActivity, idActivitySearch} from '../redux/actions';
...
function ActivityDetail(props) {
    ...
    const parseCountryList = () => {
        setCountryBox([])
        if(countryBox.length === 0){
            console.log(initialState.countryId)
            initialState.countryId.map(c=> {
                let countryToAdd = props.allCountries.filter(c=> c.id === c)[0]
                setCountryBox([...countryBox, countryToAdd])
            }) 
        }
    }
