...
function ActivityDetail(props) {
    ...
    const fetch = async () => {
        await props.idActivitySearch(idActivity)
        parseCountryList()
        handleReset()
    }
    ...
    const handleCountryChange = (e) => {
        let {id, name} = e.target.value
        if(!countryList.find((c) => c.id === id)){
    ...
    const handleClose = (e) => {
        let {id, name} = e.target.value
        countryList = countryList.filter(c => c.id !== id)
        setCountryBox(countryList)
        // setErrores(validar({...form, countryId: countryList}))
    }
