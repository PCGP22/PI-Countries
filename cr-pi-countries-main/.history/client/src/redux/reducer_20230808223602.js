...
export const reducer = (state=initialState,action) =>{  
    switch(action.type){
        ...
        case NAME_SEARCH:
            ...
            let regexpSearch = /[a-z,A-Z]{1,}/i
            console.log(regexpSearch.test(action.payload))
            if(typeof action.payload !== "string" && !regexpSearch.test(action.payload)){return{...state,countriesFiltered:[], errorMessage: "Search term must be plain text"}}
            else if(typeof action.payload === "object"){return{...state,countriesFiltered:[], errorMessage: action.payload.message}}
