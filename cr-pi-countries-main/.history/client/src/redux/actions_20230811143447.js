...
export const addCountries = () => {
    const endpoint = 'http://localhost:5000/countries/allCountries'
...
export const removeActivity = (id) => {
    const endpoint = `http://localhost:5000/countries/activities/${id}`
    return async (dispatch) => {
        try{
...
const idActivitySearch = (idActivity) => {
    const endpoint = `http://localhost:5000/countries/countries/${idActivity}`
    return async (dispatch) => {
    try{
        const {data} = await axios(endpoint)
        if(typeof data === "error"){console.log("error")}
