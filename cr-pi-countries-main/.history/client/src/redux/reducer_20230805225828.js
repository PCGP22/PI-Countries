...
export const reducer = (state=initialState,action) =>{  
    switch(action.type){
        ...
        case NAME_SEARCH:
            if(state.allCountries.filter(c=> action.payload.find(f=>f.name === c.name)) === undefined){return{
        ...
        case FILTER:
            ...
            if(action.payload[0] === "continents"){
                ...
                return{
                    ...state,
                    countriesFiltered: state.allCountries.filter(c => c.hasOwnProperty("Activities") ),
                }
            }
        case ORDER:
            console.log(action.payload)
