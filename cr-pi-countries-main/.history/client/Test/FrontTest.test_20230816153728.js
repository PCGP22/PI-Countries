...
import { addCountries, nameSearch } from '../src/redux/actions.js';
enableFetchMocks();
...
describe("Testing de searchResults",()=>{
    ...
    test("Cambia el país mostrado según la búsqueda",async()=>{

        await store.dispatch(addCountries())
        await store.dispatch(nameSearch("KEN"))
        let state = store.getState();

        render(<Provider store={store}>
                    <BrowserRouter>
                        <Container source={state.foundCountries}/>
                    </BrowserRouter>
                </Provider>)
        
        expect(await screen.getByRole("link",{ name: "Kenya" })).toBeInTheDocument();

        await store.dispatch(nameSearch("San"))
        ...
        expect(await screen.getByRole("link",{ name: "San Marino" })).toBeInTheDocument();
