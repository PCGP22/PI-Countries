...
beforeAll(() => {
  ...
  window.location = Object.defineProperties(
    ...
    test("Cambia la información dependiendo ",()=>{
        window.location.assign("http:/localhost/countries/countries/search?name=KEN")

        expect(screen.getByRole("link",{ name: "Kenya" })).toBeInTheDocument();
        expect(()=> (screen.getByRole("link",{ name: "San Marino" }))).toThrow(); //Revisa que el elemento 11 no renderice
